...
export function updateMaterial(material, config) {
  ...
  if (style !== 'mesh') {
    ...
    //     })
    //     .then((atlasJSON) => {
    //       setTextureAtlas(material, atlasJSON)
    //     })
    // }
  }
...
export function setAtlasIndexByFilename(mesh, atlasIndex, startIndex, endIndex) {
  const jsonFilename = mesh.userData.meshConfig.atlas

  if (jsonFilename) {
    // TODO what happens for multiple calls to fetch()?
    fetch(jsonFilename)
    .then((response) => {
      return response.json()
    })
    .then((atlasJSON) => {
      ...
      const index = Array.isArray(atlasJSON.frames)
        ? atlasJSON.frames.findIndex(frame => frame.filename === atlasIndex)
        : Object.keys(atlasJSON.frames).findIndex(filename => filename === atlasIndex)

      if (index < 0) {
        console.error(`unable to find atlas entry '${atlasIndex}'`)
      }

      for (let i = startIndex; i < endIndex; i++) {
        setAtlasIndexAt(mesh.geometry, i, index)
      }
